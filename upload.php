...
if (isset($_SESSION["username"])) {
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.14.0/css/all.min.cs">
...
***********************************************************/
	if (!isset($_POST['x']) && !isset($_FILES['image']['name']) ){
		//Delete users temp image
			$temppath = 'assets/imgs/profile_pics/'.$profile_id.'_temp.jpeg';
...
***********************************************************/	
	...
	//Set Upload directory    
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/First/social/assets/imgs/profile_pics';
	//Set File name	
	...
	//Move the file to correct location
		...
		if (!$move) { 
			...
			$imgSrc= "assets/imgs/profile_pics/".$file_name; // the image to display in crop area
...
if (isset($_POST['x'])){
	...
	//the file type posted
		$type = $_POST['type'];	
	//the image src
		$src = 'assets/imgs/profile_pics/'.$_POST['src'];	
	...
	if($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG'){	
		...
		//save the new cropped version
			imagejpeg($dst_r, "assets/imgs/profile_pics/".$finalname."n.jpeg", 90); 	
		...
		$result_path ="assets/imgs/profile_pics/".$finalname."n.jpeg";

		//Insert image into database
		$insert_pic_query = mysqli_query($connection, "UPDATE users SET profile_pic='$result_path' WHERE username='$logged_user'");
		header("Location: ".$logged_user);
														
}// post x
?>
<div id="Overlay"
    style=" width:100%; height:100%; border:0px #990000 solid; position:absolute; top:50%; left:50%; z-index:2000; display:none;">
</div>
